...
public class Level3Screen extends ScreenAdapter {
    ...
    private static List<Wood> woodList= new ArrayList<>();
    private static List<Pig> pigList= new ArrayList<>();
    public static int resetCount = 1;
    public String currentLevel = "Level 3";
//    private NewGameScreen newGameScreen;


    private Texture birdTexture, pigTexture, woodTexture;

    public Level3Screen(Levels levels) {
        this.levels = levels;// Initialize with the shared Levels instance
//        this.newGameScreen = newGameScreen;

    }

    public Level3Screen(int birdResetCount, List<Pig> pigList,List<Wood> woodList,Levels levels) {
        this.resetCount = birdResetCount;
...
//        NewGameScreen newGameScreen = new NewGameScreen(levels);
        ...
        world = new World(new Vector2(0, -9f), false);  // Gravity pointing downward
...
//        createDynamicBody(1300, 300, woodTexture, 1f,"wood");
        ...
        if (saveData == null) {
            // If no saved game exists, initialize a new game
...
//        woodList.add(new Wood(world, 1200, 300, woodTexture, 1f,"wood"));
//        woodList.add(new Wood(world, 1300, 300, woodTexture, 1f,"wood"));
//        Pig p1 = new Pig(world, 1200, 400, pigTexture, 0.2f,"pig");
...
//                ((Game) Gdx.app.getApplicationListener()).setScreen(new WinScreen(levels,stage,skin));  // Pass levels object
        ...
        pauseScreen = new PauseScreen(stage, skin,"Level 3");
        winScreen = new WinScreen(levels, stage, skin,"Level 3");
        loseScreen = new LoseScreen(levels, stage, skin,"Level 3");
...
//            public int resetCount = 1;
            ...
            public void touchUp(InputEvent event, float x, float y, int pointer, int button) {
                if (isDragging && !birdLaunched) {
                    dragEnd.set(x, y);
                    Vector2 force = dragStart.sub(dragEnd).scl(1000000000000000f);
//                    force.y += Math.abs(force.x); // Adjust force for a parabolic trajectory
    ...
    private void createNewGameObjects() {
        ...
        Pig p1 = new Pig(world, 1000, 500, pigTexture, 0.2f,"pig");
        Pig p2 = new Pig(world, 1300, 500, pigTexture, 0.2f,"pig");
        pigList.add(p1);
        pigList.add(p2);
    ...
    private void restoreGameState(SaveGameManager.SaveData saveData) {
        // Restore bird reset count

//        resetCount = saveData.resetCount;

        // Clear and restore pigs
        pigList.clear();
        for (SaveGameManager.PigD pigData : saveData.pigs) {
            pigList.add(new Pig(world, pigData.x, pigData.y, pigTexture, pigData.scale, pigData.type));
        }

        // Clear and restore wooden blocks
        woodList.clear();
        for (SaveGameManager.WoodD woodData : saveData.woods) {
            woodList.add(new Wood(world, woodData.x, woodData.y, woodTexture, woodData.scale, woodData.type));
        }
        for (Wood wood : woodList) {
            sprites.add(wood.getSprite());
            bodies.add(wood.getBody());
        ...
        Gdx.app.log("restoreGameState", "Game state restored: Bird resets = " + resetCount +
            ", Pigs = " + pigList.size() + ", Woods = " + woodList.size());
    }
        ...
        if (pigIndex != -1) {
            ...
            for (Pig pig : pigList) {
                if (pig.getBody().equals(pigBody)) { // Assuming Pig class has a getBody() method
                    pigList.remove(pig);  // Remove the Pig object from pigList
...
//        for (Wood wood : woodList) {
//            Texture texture = wood.getTexture();
//            Body body = wood.getBody();
//            batch.draw(
//                texture,
//                body.getPosition().x - texture.getWidth() * 0.15f / 2,
//                body.getPosition().y - texture.getHeight() * 0.15f / 2,
//                texture.getWidth() * 0.15f,
//                texture.getHeight() * 0.15f
//            );
//        }
//
//// Render pigs
//        for (Pig pig : pigList) {
//            Texture texture = pig.getTexture();
//            Body body = pig.getBody();
//            batch.draw(
//                texture,
//                body.getPosition().x - texture.getWidth() * 0.1f / 2,
//                body.getPosition().y - texture.getHeight() * 0.1f / 2,
//                texture.getWidth() * 0.1f,
//                texture.getHeight() * 0.1f
//            );
//        }
